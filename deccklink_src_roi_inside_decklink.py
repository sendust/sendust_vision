...
for decklink in list_decklink:
    for each_config in list_config:
        if (each_config["decklink"] == decklink.index_device):
            decklink.list_roi.append(rectangle(each_config["x"], each_config["y"], each_config["width"], 
            each_config["height"], name=each_config["name"], index=each_config["index"] ))


for decklink in list_decklink:
    threading.Thread(target=decklink.play).start()
    print("start pipeline play ...")
    for each_roi in decklink.list_roi:
        print(decklink.index_device , "  " , each_roi.name, "  " , each_roi.index)
