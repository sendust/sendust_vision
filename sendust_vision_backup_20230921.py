...
class rectangle:
    ...
    def get_average(self, frame):
        ...
        if ((self.accum_black or self.accum_freeze) and self.status):  # set alarm state as dirty
            self.status = 2       
...
with open("list.txt", "r") as rectangle_file:   # Read rectangle definition file
...
while True:
    ...
    if qsize > 0:
        ...
        for rect in roi_ary:
            ...
            rect.draw(hdframe)
            send_json.append({"protocol" : "status", "index" : index, "data" : rect.data_json})
            index += 1
